...
BEGIN
    ...
    ELSEIF (NEW.ID_REPARTIDOR IS NULL AND OLD.ESTADO!=2) THEN
        SIGNAL SQLSTATE '45000' SET MYSQL_ERRNO=30004, MESSAGE_TEXT='   ERROR: El pedido seleccionado no te pertenece!   ';
    END IF;
END; //
...
FOR EACH ROW
BEGIN
    IF ((SELECT COUNT(*) FROM PEDIDO WHERE ID_PEDIDO=NEW.ID_PEDIDO AND ESTADO = 2 AND ID_REPARTIDOR!=NEW.ID_REPARTIDOR) = 1) THEN
