...
BEGIN
    ...
    IF (NEW.EN_REPARTO = 1 AND @ENTREGA > NOW()) THEN
        SIGNAL SQLSTATE '45000' SET MYSQL_ERRNO=30003, MESSAGE_TEXT='ERROR: El pedido no se puede entregar hoy, pruebe con otro';
    END IF;
END; //
...
BEGIN
    IF ((SELECT COUNT(*) FROM PEDIDO WHERE ID_PEDIDO=NEW.ID_PEDIDO AND EN_REPARTO = 1 ) = 1) THEN
        SIGNAL SQLSTATE '45000' SET MYSQL_ERRNO=30004, MESSAGE_TEXT='ERROR: El pedido ha sido solicitado por otro repartidor';
    END IF;
END; // 
